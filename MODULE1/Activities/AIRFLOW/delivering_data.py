...
with dag:
    ...
    def write_dw():
        df = pd.read_csv(fn_enade_folder + 'transformed_data.csv')
        engine = sqlalchemy.create_engine(
            'mssql+pyodbc://igti-bootcamp:dados2020@127.0.0.1/enade?driver=ODBC+Driver+17+for+SQL+Server')
        df.to_sql('enade', con=engine, index=False, if_exists='replace')
    ...
    pyop_write_dw = PythonOperator(
        task_id='write_dw',
        python_callable=write_dw
    )
